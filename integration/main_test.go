...
var _ = Describe("java-buildpack-memory-calculator executable", func() {
	...
	It("executes with error when inital is not a percentage", func() {
		badFlags :=
			[]string{
				"-totMemory=2G",
				"-memoryWeights=heap:5,stack:1,permgen:3,native:1",
	...
	It("executes with error when inital too big", func() {
		badFlags :=
			[]string{
				"-totMemory=2G",
				"-memoryWeights=heap:5,stack:1,permgen:3,native:1",
		...
		Ω(string(so)).Should(BeEmpty(), "stdout not empty for "+badFlags[0])
		Ω(string(se)).Should(ContainSubstring("Initial value must be positive and no more than 100% in -memoryInitials flag; clause 'heap:101%'"), "stderr incorrect for "+badFlags[0])
	})

	It("executes with error when inital is negative", func() {
		badFlags :=
			[]string{
				"-totMemory=2G",
				"-memoryWeights=heap:5,stack:1,permgen:3,native:1",
		...
		Ω(string(so)).Should(BeEmpty(), "stdout not empty for "+badFlags[0])
		Ω(string(se)).Should(ContainSubstring("Initial value must be positive and no more than 100% in -memoryInitials flag; clause 'heap:-1%'"), "stderr incorrect for "+badFlags[0])
	})
